function Logo() {
  return (
    <svg
      width="35"
      height="30"
      viewBox="0 0 35 36"
      fill="none"
      widths={30}
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M29.5229 10.4791C32.1887 10.4791 34.3498 8.31801 34.3498 5.6522C34.3498 2.98638 32.1887 0.825317 29.5229 0.825317C26.8571 0.825317 24.696 2.98638 24.696 5.6522C24.696 8.31801 26.8571 10.4791 29.5229 10.4791Z"
        fill="#8D8BA7"
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M29.5227 22.6019C32.1885 22.6019 34.3496 20.4409 34.3496 17.7752C34.3496 15.1095 32.1885 12.9485 29.5227 12.9485C26.8569 12.9485 24.6958 15.1095 24.6958 17.7752C24.6958 20.4409 26.8569 22.6019 29.5227 22.6019Z"
        fill="#8D8BA7"
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M29.5227 35.1746C32.1885 35.1746 34.3496 33.0136 34.3496 30.3478C34.3496 27.6819 32.1885 25.5209 29.5227 25.5209C26.8569 25.5209 24.6958 27.6819 24.6958 30.3478C24.6958 33.0136 26.8569 35.1746 29.5227 35.1746Z"
        fill="#8D8BA7"
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M16.9505 22.6021C19.6162 22.6021 21.7772 20.4411 21.7772 17.7753C21.7772 15.1096 19.6162 12.9486 16.9505 12.9486C14.2848 12.9486 12.1238 15.1096 12.1238 17.7753C12.1238 20.4411 14.2848 22.6021 16.9505 22.6021Z"
        fill="#8D8BA7"
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M16.9505 35.1746C19.6162 35.1746 21.7772 33.0136 21.7772 30.3478C21.7772 27.6819 19.6162 25.5209 16.9505 25.5209C14.2848 25.5209 12.1238 27.6819 12.1238 30.3478C12.1238 33.0136 14.2848 35.1746 16.9505 35.1746Z"
        fill="#8D8BA7"
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M4.82688 35.1746C7.49269 35.1746 9.65376 33.0136 9.65376 30.3478C9.65376 27.6819 7.49269 25.5209 4.82688 25.5209C2.16107 25.5209 0 27.6819 0 30.3478C0 33.0136 2.16107 35.1746 4.82688 35.1746Z"
        fill="#8D8BA7"
      />
    </svg>
  );
}

function DropdownIcon() {
  return (
    <svg
      width="23"
      height="22"
      viewBox="0 0 23 22"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <line
        x1="21.9658"
        y1="1.02588"
        x2="1.00002"
        y2="1.02588"
        stroke="#4A3AFF"
        strokeWidth="2"
        strokeLinecap="round"
      />
      <line
        x1="21.9658"
        y1="11.0447"
        x2="1.00002"
        y2="11.0447"
        stroke="#4A3AFF"
        strokeWidth="2"
        strokeLinecap="round"
      />
      <line
        x1="21.9658"
        y1="20.9744"
        x2="1.00002"
        y2="20.9744"
        stroke="#4A3AFF"
        strokeWidth="2"
        strokeLinecap="round"
      />
    </svg>
  );
}
function EyeIcon() {
  return (
    <svg
      width="21"
      height="21"
      viewBox="0 0 21 21"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
    >
      <rect width="21" height="21" fill="url(#pattern0_64_395)" />
      <defs>
        <pattern
          id="pattern0_64_395"
          patternContentUnits="objectBoundingBox"
          width="1"
          height="1"
        >
          <use xlinkHref="#image0_64_395" transform="scale(0.02)" />
        </pattern>
        <image
          id="image0_64_395"
          width="50"
          height="50"
          xlinkHref="data:image/png;base64,"
        />
      </defs>
    </svg>
  );
}

function SearchIcon() {
  return (
    <svg
      width="25"
      height="25"
      viewBox="0 0 25 25"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M22.6564 21.5516L16.7564 15.6516C18.1742 13.9495 18.8812 11.7663 18.7303 9.55617C18.5795 7.34607 17.5823 5.27921 15.9464 3.78556C14.3104 2.29191 12.1616 1.48646 9.94695 1.53678C7.73227 1.58711 5.62226 2.48932 4.05584 4.05573C2.48942 5.62215 1.58721 7.73217 1.53689 9.94684C1.48657 12.1615 2.29201 14.3103 3.78566 15.9463C5.27932 17.5822 7.34618 18.5793 9.55628 18.7302C11.7664 18.8811 13.9496 18.1741 15.6517 16.7562L21.5517 22.6562L22.6564 21.5516ZM3.12511 10.1562C3.12511 8.7656 3.53748 7.40618 4.31009 6.2499C5.08269 5.09361 6.18082 4.1924 7.46561 3.66022C8.75041 3.12804 10.1642 2.9888 11.5281 3.2601C12.892 3.5314 14.1449 4.20107 15.1282 5.1844C16.1115 6.16774 16.7812 7.42059 17.0525 8.78452C17.3238 10.1484 17.1846 11.5622 16.6524 12.847C16.1202 14.1318 15.219 15.2299 14.0627 16.0025C12.9064 16.7751 11.547 17.1875 10.1564 17.1875C8.29219 17.1854 6.50496 16.444 5.18679 15.1258C3.86863 13.8076 3.12717 12.0204 3.12511 10.1562Z"
        fill="#8692A6"
      />
    </svg>
  );
}

function ClearIcon() {
  return(
    <svg xmlns="http://www.w3.org/2000/svg" width={25} height={25} viewBox="0 0 512 512">
      <path fill="#C3C6D1" d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM175 175c9.4-9.4 24.6-9.4 33.9 0l47 47 47-47c9.4-9.4 24.6-9.4 33.9 0s9.4 24.6 0 33.9l-47 47 47 47c9.4 9.4 9.4 24.6 0 33.9s-24.6 9.4-33.9 0l-47-47-47 47c-9.4 9.4-24.6 9.4-33.9 0s-9.4-24.6 0-33.9l47-47-47-47c-9.4-9.4-9.4-24.6 0-33.9z"/>
    </svg>
  )
}

function ArrowRightIcon() {
  return(
    <svg xmlns="http://www.w3.org/2000/svg" width={30} height={30} viewBox="0 0 320 512">
      <path fill="#4A3AFF" d="M310.6 233.4c12.5 12.5 12.5 32.8 0 45.3l-192 192c-12.5 12.5-32.8 12.5-45.3 0s-12.5-32.8 0-45.3L242.7 256 73.4 86.6c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0l192 192z" />
    </svg>
  )
}

function ArrowLeftIcon() {
  return(
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" width={30} height={30}>
      <path fill="#4A3AFF" d="M9.4 233.4c-12.5 12.5-12.5 32.8 0 45.3l192 192c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L77.3 256 246.6 86.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0l-192 192z" />
    </svg>
  )
}

function HistoryIcon() {
  return(
    <svg xmlns="http://www.w3.org/2000/svg" width={30} height={30} viewBox="0 0 512 512">
      <path d="M75 75L41 41C25.9 25.9 0 36.6 0 57.9L0 168c0 13.3 10.7 24 24 24l110.1 0c21.4 0 32.1-25.9 17-41l-30.8-30.8C155 85.5 203 64 256 64c106 0 192 86 192 192s-86 192-192 192c-40.8 0-78.6-12.7-109.7-34.4c-14.5-10.1-34.4-6.6-44.6 7.9s-6.6 34.4 7.9 44.6C151.2 495 201.7 512 256 512c141.4 0 256-114.6 256-256S397.4 0 256 0C185.3 0 121.3 28.7 75 75zm181 53c-13.3 0-24 10.7-24 24l0 104c0 6.4 2.5 12.5 7 17l72 72c9.4 9.4 24.6 9.4 33.9 0s9.4-24.6 0-33.9l-65-65 0-94.1c0-13.3-10.7-24-24-24z"/>
    </svg>
  )
}

export { DropdownIcon, Logo, EyeIcon, SearchIcon, ClearIcon, ArrowRightIcon, ArrowLeftIcon, HistoryIcon };
